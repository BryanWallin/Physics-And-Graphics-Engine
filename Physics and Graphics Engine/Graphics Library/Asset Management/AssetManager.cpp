...
bool AssetManager::loadObj(std::string filePath)
{
	...
	std::vector<float> tangents = std::vector<float>(vertexCount * 3);
	std::vector<float> bitangents = std::vector<float>(vertexCount * 3);
	...
	if (hasTextureCoordinates)
	{
		...
		for (int face = 0; face < faceCount; face++)
		{
			//Computing the tangent and bitangent vectors.
			Vector3 vertex1 = vertexList[faceVertexList[face][0] - 1];
			Vector3 vertex2 = vertexList[faceVertexList[face][1] - 1];
			Vector3 vertex3 = vertexList[faceVertexList[face][2] - 1];

			Vector2 texture1 = textureList[faceTextureList[face][0] - 1];
			Vector2 texture2 = textureList[faceTextureList[face][1] - 1];
			Vector2 texture3 = textureList[faceTextureList[face][2] - 1];

			Vector3 normal1 = normalList[faceNormalList[face][0] - 1];
			Vector3 normal2 = normalList[faceNormalList[face][1] - 1];
			Vector3 normal3 = normalList[faceNormalList[face][2] - 1];
			...
			Vector3 tangent1 = (sdir - normal1 * normal1.dot(sdir)).normalize();
			Vector3 tangent2 = (sdir - normal2 * normal2.dot(sdir)).normalize();
			Vector3 tangent3 = (sdir - normal3 * normal3.dot(sdir)).normalize();

			Vector3 bitangent1 = (normal1.cross(tangent1)).normalize();
			Vector3 bitangent2 = (normal2.cross(tangent2)).normalize();
			Vector3 bitangent3 = (normal3.cross(tangent3)).normalize();

			tangents[tangentIndex++] = tangent1[0];
			tangents[tangentIndex++] = tangent1[1];
			tangents[tangentIndex++] = tangent1[2];
			tangents[tangentIndex++] = tangent2[0];
			tangents[tangentIndex++] = tangent2[1];
			tangents[tangentIndex++] = tangent2[2];
			tangents[tangentIndex++] = tangent3[0];
			tangents[tangentIndex++] = tangent3[1];
			tangents[tangentIndex++] = tangent3[2];

			bitangents[bitangentIndex++] = bitangent1[0];
			bitangents[bitangentIndex++] = bitangent1[1];
			bitangents[bitangentIndex++] = bitangent1[2];
			bitangents[bitangentIndex++] = bitangent2[0];
			bitangents[bitangentIndex++] = bitangent2[1];
			bitangents[bitangentIndex++] = bitangent2[2];
			bitangents[bitangentIndex++] = bitangent3[0];
			bitangents[bitangentIndex++] = bitangent3[1];
			bitangents[bitangentIndex++] = bitangent3[2];
			...
			for (int vertex = 0; vertex < 3; vertex++)
			{
				...
				textureCoordinates[textureIndex++] = currentVertex[0];
				textureCoordinates[textureIndex++] = currentVertex[1];
			}
		}

		//Adding the texture coordinates, tangents, and bitangents to vertex 
		//data.
		vertexData->addData(TextureCoordinate, &textureCoordinates[0],
			vertexCount * 2);
		vertexData->addData(Tangent, &tangents[0], vertexCount * 3);
		vertexData->addData(Bitangent, &bitangents[0], vertexCount * 3);
	}
